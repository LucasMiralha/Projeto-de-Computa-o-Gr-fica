...
def start(planet_name):
    ...
    fps = 60
    if planet_name == "Arago":
        current_map = generate_arago_map()
    else:
        current_map = LEVELS.get(planet_name, LEVELS["DEFAULT"])
    level_colors = LEVEL_COLORS.get(planet_name, LEVEL_COLORS["DEFAULT"])
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                if event.key == K_ESCAPE:
                    running = False
                elif event.key == K_e and planet_name == "Arago":
        ...
        draw_floor_and_ceiling(current_map, level_colors["floor"], level_colors["ceiling"])
        ...
        hud_lines = [(f"Planeta: {planet_name}", (235, 235, 235))]
