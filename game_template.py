...
from core.renderer import draw_cube, draw_floor_tile, draw_u_stairs, draw_computer, draw_tooltip, draw_collectible_triangle
...
def start(planet):
    ...
    if planet.name in ("Cyber", "Ciber"):
        assets_dir = os.path.normpath(os.path.join(os.path.dirname(__file__), "Assets", "Textures"))
        floor_texture_id = load_texture(os.path.join(assets_dir, "cyberchao.png"))
        wall_texture_id = load_texture(os.path.join(assets_dir, "cyberwall.png"))
    ...
    running = True
    result_state = "PLAYING" # q que será retornado ao hub (vitória, quit, derrota)

    while running:
        # leitor de eventos
        for event in pygame.event.get():
            print(f"[game_template] event={event}")
            log_debug(f"[game_template] event={event}")
            if event.type == QUIT:
                result_state = "QUIT"
                running = False
                break
            ...
            if event.type == KEYDOWN:
                print(f"[game_template] keydown={event.key}")
                log_debug(f"[game_template] keydown={event.key}")
        ...
        if exit_position == (player_layer, player_row, player_col) and collected_h_points >= total_h_points:
            ...
            screen_result = show_end_screen("VITORIA", screen_width, screen_height, end_font)
        ...
        if monsters:
            ...
            for monster in monsters:
                if monster.personality == "blinky":
                    blinky_position = monster.navigation_position
                    break
            for monster in monsters:
                monster_result = monster.update(
                    ...
                    (player_layer, cam_x, cam_z),
                    current_time,
                    anchor_position=blinky_position,
                )
                if monster_result.caught_player:
                    log_debug(f"[game_template] player caught by monster at layer={monster.spawn_layer}")
                    result_state = "DERROTA"
                    screen_result = show_end_screen(
                        "VOCE PERDEU",
                        screen_width,
                        screen_height,
                        end_font,
        ...
        for y_index, andar in enumerate(current_map):
            for z_index, linha in enumerate(andar):
                for x_index, char in enumerate(linha):
                    ...
                    # desenha o chao sob todos os blocos não vazios, exceto quando estiver acima de uma escada
                    if char != ' ' and not has_ramp_below(y_index, z_index, x_index, current_map):
                        draw_floor_tile(block_x, block_y, block_z, BLOCK_SIZE, floor_texture_id)

                    # parede usa textura wall
                    if char == 'P':
                        draw_cube(block_x, block_y, block_z, BLOCK_SIZE, WALL_HEIGHT, wall_texture_id, draw_top=False)
                        # desenha o topo da parede como cyberchao para evitar falhas de teto em andares:
                        draw_floor_tile(block_x, block_y + WALL_HEIGHT - 0.01, block_z, BLOCK_SIZE, floor_texture_id)
                    ...
                    # renderização da escada
                        ...
                        draw_u_stairs(block_x, block_y, block_z, BLOCK_SIZE, WALL_HEIGHT, char, floor_texture_id)

                    elif char == 'M':
                        draw_computer(block_x, block_y, block_z, BLOCK_SIZE, WALL_HEIGHT)
                    elif char == 'H':
                        draw_collectible_triangle(
                            block_x,
